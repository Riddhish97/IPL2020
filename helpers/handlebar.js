...
module.exports = {
    ...
    helpers: {
        ...
        // For display commission in persantage and fixed 1 value after point
        commission_: function (value) {
         ...
         */
        checkAccess: function (role, access, options) {
            //check role in pre defined access configuration
            if (config.role[access].role.indexOf(role) > -1) {
                return options.fn(this);
            } else {
                return options.inverse(this);
            }
        ...
        formatMoney: function (amount, decimalCount = 2, decimal = ".", thousands = ",") {
            //This function will format number in USA currency format 
            try {
        ...
        /**     
         * Function is used for convert time into user timezone
         */
        convertToUserTz: function (time, timezone) {
            let timeZone = timezone.split("C")[1];
            return moment(time).utcOffset(timeZone).format('MM/DD/YYYY hh:mm A');
        },
        /**
         ...
         * @returns {Boolean}
         */
        checkUserProfileImagePath: function (file_name) {
            if (fs.existsSync('./public/uploads/user-avatar/' + file_name)) {
                return true;
            } else {
                return false;
            }
